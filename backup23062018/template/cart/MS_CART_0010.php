                                    ...
                                    <div class="table-responsive" id="order_table" style="width: 100%;">  
                                       <table class="table table-bordered" style="margin-bottom: 100px;">  
                                            ...
                                            <tr>  
                                                 ...
                                                 <td><?php echo $values["product_material"]; ?></td>
                                                 <td><input type="text" name="quantity[]" id="quantity<?php echo $values["product_id"]; ?>" value="<?php echo $values["product_quantity"]; ?>" data-product_price="<?= $values['product_price'] ?>" data-product_id="<?php echo $values["product_id"]; ?>" class="form-control quantity" /></td>  
                                                 ...
                                                 <td><button name="delete" class="btn btn-danger btn-xs delete" price="<?= $values['product_price'] ?>" id="<?php echo $values["product_id"]; ?>" style="margin: 0px;">Remove</button></td>  
                                            </tr>  
                                            <?php  
...
<script>  
 $(document).ready(function(data){  
      $('.add_to_cart').click(function(){  
           ...
           {  
                $.ajax({  
                     url:"../functions/action_cart_tmp2.php",  
                     method:"POST",  
                     dataType:"json",  
                     data:{  
      ...
      $(document).on('click', '.delete', function(){  
           ...
           {  
                $.ajax({  
                     ...
                     method:"POST",  
                     dataType:"json",  
                     data:{product_id:product_id, product_price:price, action:action},  
                     success:function(data){  
                          $('#order_table').html(data.order_table);  
                          $('.badge').text(data.cart_item);
      ...
      $(document).on('keyup', '.quantity', function(){  
           ...
           {  
                $.ajax({  
                     ...
                     method:"POST",  
                     dataType:"json",  
                     data:{product_id:product_id, product_price:price, quantity:quantity, action:action},  
                     success:function(data){  
                          $('#order_table').html(data.order_table);  
                     }  
                });  

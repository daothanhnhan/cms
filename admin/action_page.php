...
class action_page extends library{
	...
	/*--------------- Sản phẩm --------------------*/
		...
		public function addProduct(){
			...
			$friendly_url = isset($_POST['friendly_url']) ? $_POST['friendly_url'] : '';
			////////////
			$product_gift = isset($_POST['product_gift']) ? $_POST['product_gift'] : 0;
			$product_gift_id = isset($_POST['product_gift_id']) ? $_POST['product_gift_id'] : 0;
			////////////////////
			...
			$arr_product_color = array();
			foreach ($product_color as $item) {
				if ($item != '') {
					$arr_product_color[] = $item;
				}
			}
			$product_color = json_encode($arr_product_color);
			//////////////////////////
			$product_price_sub = isset($_POST['product_price_sub']) ? $_POST['product_price_sub'] : '';
			$arr_price_sub_pro = array();
			foreach ($product_price_sub as $item) {
				// if ($item != '') {
					$arr_price_sub_pro[] = $item;
				// }
			}
			$product_price_sub = json_encode($arr_price_sub_pro);
			//--------------
			$arr_product_material = array();
			foreach ($product_material as $item) {
				// if ($item != '') {
					$arr_product_material[] = $item;
				// }
			}
			$product_material = json_encode($arr_product_material);
			/////////////////////////
   ...
   //          }
			...
			if (isset($_POST['producttag_id'])) {
				...
				$producttag_id = "";
				$arr_producttag = $_POST['producttag_id'];
				foreach ($arr_producttag as $item) {
					$producttag_id .= $item . ',';
				}
			} else {
				$producttag_id = '';
	  ...
	  //   	if(isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != ""){
			...
			$insert = $this->Insert_1('product', 
									   ...
									   'sssssssdddsssisiiissssssssssssssssissssisssii');
   ...
   //          }
			...
			if (!$this->Update_1('product',
								  ...
								  'sssssssdddsssisiiissssssssssssssssisssssssiii')) {
...
// echo json_encode(array('error'=>'Lỗi, mời thử lại'));die;
    	...
    	public function updateLangService(){
    		...
    		foreach ($lang as $key => $value) {
// echo json_encode(array('error'=>'Lỗi, mời thử lại check'.$value['lang_service_sub_info1']));die;
...
/*---------------- Danh mục dịch vụ --------------------*/
	...
	/*------------------ dịch vụ -------------------*/
		...
		public function listProductGift () {
			global $conn_vn;
			$sql = "SELECT * FROM product WHERE product_gift = 1";
			$result = mysqli_query($conn_vn, $sql);
			$rows = array();

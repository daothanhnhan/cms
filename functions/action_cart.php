...
class action_cart extends library{
       ...
       {  
            ...
            if($_POST["action"] == "add"){  
              ...
              {  
                ...
                foreach($_SESSION["shopping_cart"] as $keys => $values)  
                {  
                  if($_SESSION["shopping_cart"][$keys]['product_id'] == $_POST["product_id"] && $_SESSION['shopping_cart'][$keys]['product_price'] == $_POST['product_price'])  
                  {  
                    $is_available++;  
                    $_SESSION["shopping_cart"][$keys]['product_quantity'] = $_SESSION["shopping_cart"][$keys]['product_quantity'] + $_POST["product_quantity"];  
                  }  
  ...
  /*---------------------------------------------------*/
  public function add_cart3(){
    global $conn_vn;
    $sql = "SELECT * FROM discount_code WHERE code = '" . $_POST['code_sale'] . "'";
    ...
    if ($num > 0) {
      ...
      $discount = $row['number'];
      if ($type == 1) {
        $product_price = (int)$_POST['product_price']*(1-($discount/100));
      } elseif ($type == 2) {
        $product_price = $_POST['product_price'] - $discount;
      } else {
        $product_price = $_POST['product_price'];
      }
    } else {
  ...
  /*-------------------------------------------------*/
  public function updateCartTable1($total_price,$id_cart,$scores){
    global $conn_vn;
    $date_cart = $this->sw_get_current_weekday();
    $diem = 0;
    ...
    $sql = "UPDATE cart SET total_price = '$total_price', scores = $diem WHERE id_cart = $id_cart";
    $result = mysqli_query($conn_vn,$sql);  
    return 1;
  }
  /*-------------------------------------------------*/
  ...
  public function payment2 () {
      ...
      if(isset($_POST['txtAddress'])) $address_buyer = $_POST['txtAddress'];
      if(isset($_POST['txtNote'])) $note_buyer = $_POST['txtNote'];
      $scores = (isset($_POST['scores'])) ? $_POST['scores'] : 0;
      $totalPrice = 0;
      ...
      if($this->updateCartTable1($totalPrice,$id_cart,$scores)){
          $this->emptyCart();
          session_regenerate_id();
          //echo json_encode(array("success"=>"Gửi đơn hàng thành công", "totalCart"=>$this->countCart(), "url"=>"/"));
          echo '<script>alert(\'Gửi đơn hàng thành công\');</script>';
  ...
  public function scores () {
    ...
    if (isset($_SESSION['user_id_gbvn'])) {
      ...
      if (isset($_POST['scores'])) {
        ...
        $sql = "UPDATE user SET scores = $diem WHERE id = $id";
        $result = mysqli_query($conn_vn, $sql);
      } else {
        $price = (int)($price/10);
        $price = $price + $scores;

        $sql = "UPDATE user SET scores = $price WHERE id = $id";
        $result = mysqli_query($conn_vn, $sql);
      }      
    }    
  }

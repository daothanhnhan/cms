...
class action_lang extends library
{
	public  function get_url_lang_newscat ($url, $langu) {
        ...
        $sql = "SELECT * FROM $this->nameTable_newsCatLanguage Where $this->nameColType_newsCatLanguage = '$langu' And $this->nameColUrlNewsCat_newsCatLanguage = '$url'";//echo $sql;
        $result = mysqli_query($conn_vn, $sql) or die('error1: ' . mysqli_error($conn_vn));
        ...
        $sql = "SELECT * FROM $this->nameTable_newsCatLanguage Where $this->nameColType_newsCatLanguage = '$lang' And $this->nameColIdNewsCat_newsCatLanguage = ".$row['newscat_id']; //echo $sql;
